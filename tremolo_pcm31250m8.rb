File::open(__FILE__ + ".wav","w+b") {|file|
  ...
  envelope_lead = [50,1000,15,10000,750,750,250]
...
#    0.92968750, 0.87890625, 0.82812500, 0.78125000, 0.73828125, 0.69531250, 0.65625000, 0.62109375, 0.58593750, 0.55468750, 0.52343750, 0.49218750, # C6-
  ...
  data_size = sample_rate * (8 + 8) * 120 / tempo
  ...
  melody.each { |note_number, length|
    ...
    (0...length).each{ |count|
      ...
      when D
        if envelope_generator_rest <= 0
          if envelope_generator_level > envelope[S]
            envelope_generator_level -= 1
            envelope_generator_rest = envelope[envelope_generator_state]
          else
            envelope_generator_state = TN
            envelope_generator_rest = envelope[envelope_generator_state]
          end
        end
      when TN
        if envelope_generator_rest <= 0
          envelope_generator_state = TD
          envelope_generator_rest = envelope[envelope_generator_state]
        end
      when TD
        if envelope_generator_rest <= 0
          if envelope_generator_level > envelope[S] - 3
            envelope_generator_level -= 1
            envelope_generator_rest = envelope[envelope_generator_state]
          else
            envelope_generator_state = TU
            envelope_generator_rest = envelope[envelope_generator_state]
          end
        end
      when TU
      ...
      if (envelope_generator_state <= TU) && (count >= (length * 3) / 4)
