File::open(__FILE__ + ".wav","w+b") {|file|
  ...
  wavemem_noise_a  = [10,4,3,29,18,25,19,0,8,6,9,12,21,22,11,14,30,23,27,15,16,2,13,24,7,26,1,31,5,17,28,20]
...
#    0.92968750, 0.87890625, 0.82812500, 0.78125000, 0.73828125, 0.69531250, 0.65625000, 0.62109375, 0.58593750, 0.55468750, 0.52343750, 0.49218750, # C6-
  ...
  data_size = sample_rate * (8 + 4) * 120 / tempo
  ...
  melody.each { |note_number, length|
    ...
    (0...length).each{ |count|
      ...
      mix_sub_2 = 0
      if note_number > 60
        mix_sub_2 = (note_number - 60)
      end
      wavemem_step_sub_2 = wavemem_step + 0 # phase shift
      wavemem_step_sub_2 %= wavemem_resolution

      level = ((wavemem[wavemem_step] * (31 - mix_sub) / 32 +
                wavemem_sub[wavemem_step_sub] * mix_sub / 32) * (31 - mix_sub_2) / 32 +
               (wavemem_sub_2[wavemem_step_sub_2] * mix_sub_2 / 32)) *
